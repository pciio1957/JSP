	...
	<table id="showCart" align="center" class="listTable" width="70%">
		...
		<% } else { %>
			<% for(int idx=0; idx<clist.size(); idx++) {
				...
				<tr><td><%= idx+1 %></td><td><%= pro.getName() %></td><td><%= pro.getPrice() %></td>
				<td><%= pro.getCnt() %></td><td><%= pro.getPrice()*pro.getCnt() %></td></tr>
		<% }} %>
		</tbody>
		<tfoot><th colspan="3">총계<th><td id="tot"></td></tfoot>
	</table>	
	<h3 align="center" onclick="location.href='a0804_2_session.jsp'">물건보기</h3>
</body>
</html>