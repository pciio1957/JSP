...
4. [js] 전날한 생성자 class를 선언하고, 객체를 생성하는 형식을 활용하여, 장바구니에 담은 상품을 누적처리하세요.
    ...
    1) 초기화면
    	요청값 없음 if(name != null) 		세션(cartList) 없음
    2) 첫번째 데이터를 입력하고 장바구니 담기
		요청값 전달 sumbit을 통해서			요청값을 가져온다 (객체생성 new Product)  세션(cartList) 없음
								    	if(clist==null) { 세션없는 것을 check해서  
								    	clist=new ArrayList<Product>(); 만들고 
								    	생성된 객체는 clist에 할당. clist.add(prod)
								    	세션(cartList) 세션값이 생성
								    	현재 세션값이 있기에 ArrayList<Product> clist = (ArrayList<Product>) session.~
								    	로 가져올 수 있다
								    	clist.add(생성객체 할당)  : 기존에 있는 내용에 추가
								 		다시 session값에 할당 
...
<%
	...
	if(name != null) {
		...
		clist.add(prod); // session에 추가할 장바구니를 처리한다.
		System.out.println("장바구니현황 : " + clist.size());
...
<body>
	...
	<h3 align="center" onclick="location.href='a0804_3_show_session.jsp'">장바구니확인하기</h3>
</body>
</html>