...
4. el과 jstl을 활용하여 기본 경로 설정과 한글 post방식의
	...
	<c:set var="path" value="${pageContext.request.contextPath}"/>
		-> 서버 안에 자원(소스 및 이미지lib)에 접근할 때 기준 경로를 설정할 수 있다
	<fmt:requestEncoding value="utf-8"/>
...
5. XMLHttpRequest는 어떤 역할을 하는 객체인지 기술하기
	...
	- 클라이언트(브라우저)와 서버(톰캣)간의 요청값을 전송하거나
	반응값을 받을 때 화면 전환없이 처리하게 해주는 js의 객체이다
...
6. 동기와 비동기 통신 방식의 차이를 기술하고, XMLHttpRequest에서
	...
	1) 동기 방식
	클라이언트가 특정한 자원을 서버에 요청하면 그 데이터가 올때까지 
	다른 작업을 하지않고 있다가 데이터가 오면 처리하는 방식을 말함
	xhr.open("", "", false);
	
	
	2) 비동기 방식
	클라이언트가 서버에 특정 자원을 요청하는 이벤트 핸들러를 만들고
	해당 이벤트 핸들러에 결과값이 올 때를 대비하여 처리해놓고 
	다른 작업을 수행할 수 있도록 하는 방식을 말함
	xhr.open("", "", true);
...
8. json 데이터의 기본형식 기술하기
	반응 값으로 json 데이터를 전달해서 js스크립트로 사용될 때 꼭 지켜야할 형식 선언
...
<body>
	...
	<h3 align="center">${(not empty param.userid)?"아이디 입력":"아이디 입력 안됨"}</h3>
